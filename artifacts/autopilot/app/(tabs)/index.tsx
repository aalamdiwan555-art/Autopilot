...
export default function HomeScreen() {
  ...
  const completed = setupComplete ? drivers.length : drivers.filter((driver) => driver.connected).length;

  const loadState = useCallback(async () => {
    try {
      const stored = await AsyncStorage.getItem('autopilot-running');
      const storedSetup = await AsyncStorage.getItem('autopilot-setup-complete');
  ...
  const completeSetup = async () => {
    if (setupStep >= setupCopy.length - 1) {
      await AsyncStorage.setItem('autopilot-setup-complete', 'true');
      ...
      >
        ...
        <Animated.View style={[styles.driverList, entranceStyle]}>
          {drivers.map((driver, index) => (
            <Pressable
              key={driver.name}
              testID={`driver-${driver.name.toLowerCase()}`}
              accessibilityRole="button"
              accessibilityLabel={`${driver.name}, ${driver.connected ? 'connected and ready' : 'needs setup'}`}
              onPress={() => setShowSetup(true)}
              style={({ pressed }) => [styles.driverRow, index === drivers.length - 1 && styles.driverRowLast, pressed && styles.pressed]}
            >
